...
return <div className = '  rounded-lg flex items-center gap-4 '>
  <div  className = {`size-${size} rounded-full ${info.online ? "bg-gradient-to-br from-blue-400 to-blue-100" : "bg-neutral-300"} flex items-center justify-center`}>
    ...
    <img onLoad = {() => setImgLoaded(true)} src = {info?.icon || 'https://upload.wikimedia.org/wikipedia/commons/a/ac/Default_pfp.jpg'} className = {`size-${size > 30 ? size - 2 : size - 1} ${imgLoaded ? "visible" : "hidden"} rounded-full object-cover`} />
  </div>
